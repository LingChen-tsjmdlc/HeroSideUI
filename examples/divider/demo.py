...
class DividerDemo(DemoBase):
    ...
    def build_content(self, layout: QVBoxLayout, labels_bag: list):
        # 1. 默认水平分割线
        self.add_section_vertical(layout, "水平分割线 (默认)", [
            QLabel("上方内容"),
            Divider(),
            QLabel("下方内容"),
        ], labels_bag)
        ...
        self.add_section_vertical(layout, "自定义颜色分割线", [
            Divider(color=c) for c in colors
        ], labels_bag, spacing=4)

        # 3. 带文字的分割线
        self.add_section_vertical(layout, "带文字的分割线", [
            QLabel("Sign in with your account"),
            Divider(text="OR"),
            QLabel("Sign in with social providers"),
