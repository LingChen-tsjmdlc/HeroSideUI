...
class InputDemo(DemoBase):
    ...
    def build_content(self, layout: QVBoxLayout, labels_bag: list):
        # 4 变体 × 6 颜色
        for v in VARIANTS:
            self.add_full_width(layout, f"Variant: {v}", grid_of_inputs(v), labels_bag)
        ...
        self.add_full_width(
            layout, "3 sizes (variant=flat, color=primary)", size_row, labels_bag
        )
        ...
        for lp in ["inside", "outside", "outside-top", "outside-left"]:
            self.add_full_width(
                layout,
                f"label_placement = {lp}",
                grid_of_inputs("flat", lp),
                labels_bag,
            )
        ...
        self.add_full_width(
            layout,
            "is_disabled",
            Input(label="Disabled", variant="flat", is_disabled=True),
            labels_bag,
        )
        ...
        self.add_full_width(
            ...
            "is_readonly",
            Input(
                label="Readonly", value="这是只读内容", variant="flat", is_readonly=True
            ),
            labels_bag,
        )
        ...
        self.add_full_width(
            ...
            "is_required",
            Input(
                label="Username", variant="bordered", color="primary", is_required=True
            ),
            labels_bag,
        )
        ...
        invalid_widgets = [
            Input(
                ...
                variant=v,
                is_invalid=True,
                error_message="请输入合法的邮箱地址",
            )
            for v in VARIANTS
        ]
        self.add_full_width_group(
            layout, "is_invalid + error_message", invalid_widgets, labels_bag
        )

        # start/end content
        self.add_full_width(
            layout,
            "start_content = string icon",
            Input(
                label="Search",
                variant="flat",
                color="default",
        ...
        self.add_full_width(
            layout, "字符串图标 + 点击回调 (密码框眼睛)", pwd, labels_bag
        )
        ...
        # 事件
        ev = Input(
            label="Type here", variant="flat", color="primary", is_clearable=True
        )
