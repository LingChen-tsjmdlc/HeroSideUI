...
class TabsDemo(DemoBase):
    ...
    def build_content(self, layout: QVBoxLayout, labels_bag: list):
        ...
        for i, c in enumerate(
            ...
            grid.addWidget(Body(c), i // 3, (i % 3) * 2)
        ...
        for i, c in enumerate(
            ...
            grid2.addWidget(Body(c), i // 3, (i % 3) * 2)
        ...
        for s in ["sm", "md", "lg"]:
            size_rows.append(
                labeled_row(
                    f"{s}:",
                    Tabs(["Photos", "Music", "Videos"], size=s, color="primary"),
                )
            )
        self.add_full_width_group(
            layout, "3 sizes (sm / md / lg)", size_rows, labels_bag
        )
        ...
        self.add_full_width_group(
            layout, "5 radius (none/sm/md/lg/full)", radius_rows, labels_bag
        )
        ...
        self.add_full_width(
            layout, "4 placements (top / bottom / start / end)", place_w, labels_bag
        )
        ...
        t_icon.add_tab(
            "Next", panel("Next panel"), start_icon="heroicons--chevron-right-solid"
        )
        t_icon.add_tab("Show", panel("Show panel"), end_icon="heroicons--eye-solid")
        t_icon.add_tab(
            "Hide",
            panel("Hide panel"),
        ...
        t_icon2.add_tab(
            "Next", panel("Next"), end_icon="heroicons--chevron-right-solid"
        )
        t_icon2.add_tab("Hide", panel("Hide"), start_icon="heroicons--eye-slash-solid")

        self.add_full_width_group(
            layout,
            "TabItem 档 2 — start_icon / end_icon",
            [t_icon, t_icon2],
            labels_bag,
        )
        ...
        t_custom.add_tab(
            custom=self._dot_widget("📬", "Notifications"),
            content=panel("Notifications"),
            key="noti",
        )
        t_custom.add_tab(
            custom=self._dot_widget("💬", "Messages", "#f5a524"),
            content=panel("Messages"),
            key="msg",
        )
        t_custom.add_tab("Trash", panel("Trash"))
        self.add_full_width(
            layout, "TabItem 档 3 — custom widget", t_custom, labels_bag
        )
    ...
    def _dot_widget(
        self, emoji: str, label: str, dot_color: str = "#f31260"
    ) -> QWidget:
        w = QWidget()
        lay = QHBoxLayout(w)
        lay.setContentsMargins(6, 0, 6, 0)
        lay.setSpacing(6)
        # emoji 图形：必须用 Segoe UI Emoji 等专用字体，不走 Text/FontProvider 体系
        ic = QLabel(emoji)
        ic.setFont(QFont("Segoe UI Emoji", 13))
        lay.addWidget(ic)
        # 文字承载：用 Text 体系的 Body，享受字体/主题统一
        tx = Body(label)
        lay.addWidget(tx)
        # 状态指示圆点：QLabel 当画布用，无文字内容，纯靠 stylesheet 上色
        dot = QLabel()
