...
def main():
    ...
    window = QMainWindow()
    window.setWindowTitle("Tooltip 示例 - 亮色模式")
    ...
    title = QLabel("Tooltip - Light Mode")
    ...
    for c in colors:
        ...
        tooltip = Tooltip(content=f"{c} tooltip", color=c)
        tooltip.attach(btn)
        tooltips.append(tooltip)
    ...
    for offset_val in [0, 3, 7, 14, 24]:
        btn = Button(f"offset={offset_val}", color="primary", variant="flat")
        tooltip = Tooltip(
    ...
    # 默认 (0/150)
    btn1 = Button("默认 (0/150ms)", color="success", variant="flat")
    t1 = Tooltip(content="open_delay=0, close_delay=150", color="success")
    t1.attach(btn1)
    tooltips.append(t1)
    delay_row.addWidget(btn1)

    # 慢开 (500/150)
    btn2 = Button("慢开 (500/150ms)", color="warning", variant="flat")
    t2 = Tooltip(content="open_delay=500, close_delay=150", color="warning", open_delay=500)
    t2.attach(btn2)
    tooltips.append(t2)
    delay_row.addWidget(btn2)

    # 快关 (0/0)
    btn3 = Button("快关 (0/0ms)", color="danger", variant="flat")
    t3 = Tooltip(content="open_delay=0, close_delay=0", color="danger", close_delay=0)
    t3.attach(btn3)
    tooltips.append(t3)
    delay_row.addWidget(btn3)

    # 慢开慢关 (300/500)
    btn4 = Button("慢速 (300/500ms)", color="secondary", variant="flat")
    t4 = Tooltip(content="open_delay=300, close_delay=500", color="secondary", open_delay=300, close_delay=500)
    t4.attach(btn4)
    tooltips.append(t4)
    ...
    btn_no_arrow = Button("无箭头", color="primary", variant="flat")
    t_no = Tooltip(content="没有箭头", color="primary", show_arrow=False)
    t_no.attach(btn_no_arrow)
    tooltips.append(t_no)
    arrow_row.addWidget(btn_no_arrow)

    btn_arrow = Button("有箭头", color="primary", variant="flat")
    t_yes = Tooltip(content="带箭头指向 trigger", color="primary", show_arrow=True)
    ...
    for sz in ["sm", "md", "lg"]:
        ...
        tooltip = Tooltip(content=f"Size: {sz}", size=sz)
        tooltip.attach(btn)
        tooltips.append(tooltip)
    ...
    for r in ["none", "sm", "md", "lg", "full"]:
        ...
        tooltip = Tooltip(content=f"Radius: {r}", radius=r, color="primary")
        tooltip.attach(btn)
        tooltips.append(tooltip)
    ...
    tooltip_rich = Tooltip(color="secondary", placement="bottom", show_arrow=True)
    ...
    tooltip_icon = Tooltip(color="success", placement="bottom", show_arrow=True)
    ...
    btn_dynamic = Button("点击 +1 (count=0)", color="warning", variant="flat")
    tooltip_dyn = Tooltip(content="当前计数: 0", color="warning", placement="bottom", show_arrow=True)
